...
namespace
{
    ...
    void SetWeaponSlot(Inventory::WeaponSlotInfo& weaponSlot, const std::string& weaponToLoad)
    {
...
Inventory::Inventory(void) :
    ...
    m_inventory { 
                    WeaponSlotInfo( LAST_STAND, "BaseWeapon.uatype"),
...
void Inventory::OnInitialize(void)
{
    ...
    ursine::ecs::Entity* arm = GetOwner( )->GetChildByName("RightArm");
...
{
    SetWeaponSlot( m_inventory[ LAST_STAND ], archetype );
}
...
{
    SetWeaponSlot(m_inventory[ SECONDARY_WEAPON ], archetype);
}
...
{
    SetWeaponSlot(m_inventory[ PRIMARY_WEAPON ], archetype);
}
...
{
    SetWeaponSlot( m_inventory[ MELEE_WEAPON ], archetype );
}
