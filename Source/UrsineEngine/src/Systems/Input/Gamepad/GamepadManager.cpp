...
namespace Ursine
{
    ...
    void GamepadManager::Update(void)
    {
        ...
        for (uint i = 0; i < m_gamepads.size( ); ++i)
        {
            ...
            gamepad.state.m_sticksCurrent.applyDeadZone( gamepad.dz_type, DEADZONE_SIZE );
    ...
    GamepadState *GamepadManager::GetState(GamepadIndex index)
    {
    ...
    VirtualGamepad GamepadManager::CreateVirtualGamepad(void)
    {
        ...
        return { m_virtualGamepads.size( ) - 1u };
    }

    VirtualGamepad GamepadManager::GetVirtualGamepad(GamepadIndex index)
    {
        UAssert( IsVirtual( index ), "Gamepad isn't virtual." );

        return { IndexFromVirtual(index) };
    }
    ...
    void GamepadManager::onGamepadAdded(EVENT_HANDLER(Application))
    {
        ...
        gamepad.instance_id = SDL_JoystickInstanceID( gamepad.joystick );
    ...
    {
        m_virtualGamepads[ index ].is_valid = false;
    }
}
