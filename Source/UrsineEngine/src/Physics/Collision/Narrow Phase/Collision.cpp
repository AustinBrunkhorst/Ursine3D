...
namespace Ursine
{
    ...
    {
        Vec2 support_A = collider_A.shape->GetSupport(direction);
        Vec2 support_B = collider_B.shape->GetSupport(-direction);
        Vec2 vert = support_A - support_B;
    ...
    SimplexEdge EPAFindClosestEdge(const std::vector<Simplex::Vert> &epa_simplex)
    {
        ...
        for (uint i = 0; i < epa_simplex.size(); ++i)
        {
            ...
            const Vec2 &A = epa_simplex[i].vert;
            const Vec2 &B = epa_simplex[j].vert;
        ...
        const Simplex &simplex, Contacts &contacts)
    {
        ...
        for (int i = 0; i < 100; ++i)
        {
            ...
            float dist = Vec2::Dot(support, edge.normal);
    ...
    bool IsColliding(const Collider &collider_a, const Collider &collider_b, Contacts &contacts)
    {
        ...
        Vec2 d = Vec2(1, -1);//Vec2(collider_b.root_trans.PositionWC() - collider_a.root_trans.PositionWC());
        ...
        while (count < 100)
        {
            ...
            if (Vec2::OppositeDirection(simplex.A().vert, d))
            {
