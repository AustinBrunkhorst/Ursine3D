...
namespace Ursine
{
    ...
    {
        return Texture2D::FromFile( name );
    }
    ...
    void JsonSerializer::Deserialize(const Json &data, Texture2D * &out)
    {
        if (data.is_null( ))
            ...
            out = gContentManager->Load<Texture2D>( data.string_value( ) );
    }

    template<>
    Json JsonSerializer::Serialize(std::vector<Texture2D *> &instance)
    {
        Json::array text_array;

        for (auto &text : instance)
        {
            text_array.push_back(Json(text->GetPath()));
        }
    ...
    {
        UAssert(data.is_array(), "Textures must be an array of strings (i.e. \"textures\": [ ...... ] )");

        for (auto &item : data.array_items())
        {
            out.push_back(gContentManager->Load<Texture2D>(item.string_value()));
        }
    }
}
