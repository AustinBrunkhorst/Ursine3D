...
namespace rey_oso
{
  namespace DXCore
  {
    void DepthStateManager::Initialize( ID3D11Device *device, ID3D11DeviceContext *devicecontext )
    {
      m_device = device;
    ...
    ID3D11DepthStencilState *DepthStateManager::GetStencilState( DEPTH_STATES state )
    {
      return m_depthStateArray[ state ];
    }

    void DepthStateManager::SetDepthState( DEPTH_STATES state )
    {
