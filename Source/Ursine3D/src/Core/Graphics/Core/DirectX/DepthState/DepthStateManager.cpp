...
namespace ursine
{
  void DepthStateManager::Initialize( ID3D11Device* device, ID3D11DeviceContext* devicecontext, int width, int height )
  {
    m_device = device;
  ...
  void DepthStateManager::Uninitialize( )
  {
  ...
  ID3D11DepthStencilView* DepthStateManager::GetDepthStencilView( DEPTH_STENCIL_LIST stencil )
  {
    return m_depthStencilViewArray[ stencil ];
  }

  ID3D11ShaderResourceView* DepthStateManager::GetDepthStencilSRV( DEPTH_STENCIL_LIST stencil )
  {
    return m_depthStencilResourceArray[ stencil ];
  }

  void DepthStateManager::Resize ( int width, int height )
  {
    for (auto x : m_depthStencilTextureArray)
    {
      RELEASE_RESOURCE( x );
