	...
	namespace graphics
	{
		namespace ufmt_loader
		{
			void FbxModel::GetFinalTransform(const std::string& clipName, double timePos, std::vector<XMMATRIX>& finalTransform) const
			{
				...
				{
					XMVECTOR S = XMLoadFloat3(&mSkinnedData.mbonehierarchy[i].boneSpaceScaling);
					XMVECTOR P = XMLoadFloat3(&mSkinnedData.mbonehierarchy[i].boneSpacePosition);
					XMVECTOR Q = XMLoadFloat4(&mSkinnedData.mbonehierarchy[i].boneSpaceRotation);
		...
		XMVECTOR Utilities::SetFloat3ToXMVector(const XMFLOAT4& rhs)
		{
			XMVECTOR ret;
