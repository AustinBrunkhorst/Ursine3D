		...
		namespace ufmt_loader
		{
			...
			bool MeshInfo::SerializeIn(HANDLE hFile)
			{
				...
				if (hFile != INVALID_HANDLE_VALUE)
				{
					...
					//materials = new Material[mtrlCount];
					//for (i = 0; i < mtrlCount; ++i)
					//{
					//	ReadFile(hFile, &materials[i], sizeof(Material), &nBytesRead, nullptr);
					//}
					for (i = 0; i < mtrlCount; ++i)
					{
					...
					modelSubsets = new ModelSubset[subsetCount];
					for (i = 0; i < subsetCount; ++i)
					{
						ReadFile(hFile, &modelSubsets[i], sizeof(ModelSubset), &nBytesRead, nullptr);
					}
				}
				return true;
			}
			...
			bool MeshInfo::SerializeOut(HANDLE hFile)
			{
				...
				if (hFile != INVALID_HANDLE_VALUE)
				{
					...
					//for (i = 0; i < mtrlCount; ++i)
					//{
					//	WriteFile(hFile, &materials[i], sizeof(Material), &nBytesWrite, nullptr);
					//}
					for (i = 0; i < mtrlCount; ++i)
					{
					...
					for (i = 0; i < subsetCount; ++i)
					{
						WriteFile(hFile, &modelSubsets[i], sizeof(ModelSubset), &nBytesWrite, nullptr);
					}
				}
				return true;
			};
